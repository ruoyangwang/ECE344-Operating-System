...
struct thread {
	...
	//struct lock * threadlock;	//unique pid for thread
	
	struct lock * lk;
	struct cv * Waitonexit, *Waittoexit;
	struct thread * parent;
	struct thread * child[20];
	int exitcode;
	int status;
	int pid;
	int waiting_child;
...
/*struct pid_linklist
{
	struct lock *nodelock;
	struct cv* cv;
	struct pid_linklist *next;
	struct thread* the_parent;
	struct thread* the_child;
	int pstate;
	int cstate;
	int exitcode;
	int waitcode;
	int checked;	//checked indicates whether this child thread has been checked for once or not.
};
struct pid_linklist *pid_h ;*/
